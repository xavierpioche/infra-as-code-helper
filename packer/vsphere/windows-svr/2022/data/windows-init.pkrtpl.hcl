...
$adapter | New-NetIPAddress -AddressFamily IPV4 -IPAddress ${vm_inst_ip} -PrefixLength ${vm_inst_ip_prefix} -DefaultGateway ${vm_inst_ip_gw}
$adapter | Set-DnsClientServerAddress -ServerAddresses ("${vm_inst_ip_dns1}","${vm_inst_ip_dns2}")
$adapter | Set-DnsClient -ConnectionSpecificSuffix "${vm_inst_ip_domain}" -RegisterThisConnectionsAddress $true -UseSuffixWhenRegistering $true

$regKey="HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings"
Set-ItemProperty -path $regKey ProxyEnable -value ${vm_inst_proxy_onoff}
Set-ItemProperty -path $regKey ProxyServer -value ${vm_inst_proxy_ip}:${vm_inst_proxy_port}
